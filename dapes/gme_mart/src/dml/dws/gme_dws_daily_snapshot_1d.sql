-- DML: Daily summary for briefing skill
-- Corresponding DDL: src/ddl/dws/gme_dws_daily_snapshot_1d.sql
...
pc_ratio AS (
    SELECT pull_date, ticker,
        ...
        / NULLIF(SUM(CASE WHEN option_type = 'call' THEN open_interest ELSE 0 END), 0) AS pc_ratio
    FROM {{ ref('gme_dwd_option_contract_di') }}
    GROUP BY pull_date, ticker
),

top_oi AS (
    SELECT pull_date, ticker, strike, open_interest,
...
SELECT
    ...
    ga.top_gex_strike,
    pc.pc_ratio,
    (SELECT strike FROM top_oi WHERE pull_date = s.pull_date AND ticker = s.ticker AND oi_rank = 1) AS top_oi_strike_1,
    (SELECT strike FROM top_oi WHERE pull_date = s.pull_date AND ticker = s.ticker AND oi_rank = 2) AS top_oi_strike_2,
    (SELECT strike FROM top_oi WHERE pull_date = s.pull_date AND ticker = s.ticker AND oi_rank = 3) AS top_oi_strike_3
