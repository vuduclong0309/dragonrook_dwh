...
Mirrors gme_mart/src/resource/run_ddl.py — runs BEFORE dbt to ensure target schemas exist.
...
def get_connection(target: str):
    if target == "cloud":
        ...
        con.execute("CREATE DATABASE IF NOT EXISTS xauusd_db")
        con.close()
...
def main():
    parser = argparse.ArgumentParser(description="Run DDL for XAUUSD Mart")
    ...
    print(f"\n[VERIFY] Tables in xauusd_db: {[t[0] for t in tables]}")
    con.close()
